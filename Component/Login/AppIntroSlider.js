...
export default class AppIntroSlider extends React.Component {
  ...
  _autoPlay = () =>{
    const isAutoPlay = this.props.autoPlay
    const maxPlay = this.props.maxPlay - 1
    const timerNext = this.props.timerNext === undefined ? 500 : this.props.timerNext
      if(isAutoPlay === true && maxPlay > 0){
          this.timer = setTimeout(()=>{
            this._onNextPress()
            if(this.state.activeIndex < maxPlay){
              this._autoPlay()
            }
          },timerNext)
      }
